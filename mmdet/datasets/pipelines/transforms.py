...
class TwoRandomFlip:
    ...
    def __call__(self, results):
        ...
        if 'flip' not in results:
            ...
            strong_cur_dir = np.random.choice(direction_list, p=flip_ratio_list)
            weak_cur_dir = np.random.choice(direction_list, p=flip_ratio_list)

            results['flip'] = [strong_cur_dir is not None, weak_cur_dir is not None]
        if 'flip_direction' not in results:
            results['flip_direction'] = [strong_cur_dir, weak_cur_dir]
        results['img'] = [results['img'], results['img']]
        if results['flip'][0]:
            results['img'][0] = mmcv.imflip(
                results['img'][0], direction=results['flip_direction'][0])
        if results['flip'][1]:
            # flip image
            for key in results.get('img_fields', ['img']):
                results[key][1] = mmcv.imflip(
                    results[key][1], direction=results['flip_direction'][1])
            # flip bboxes
            for key in results.get('bbox_fields', []):
                results[key] = self.bbox_flip(results[key],
                                              results['img_shape'],
                                              results['flip_direction'][1])
            # flip masks
            for key in results.get('mask_fields', []):
                results[key] = results[key].flip(results['flip_direction'][1])

            # flip segs
            for key in results.get('seg_fields', []):
                results[key] = mmcv.imflip(
                    results[key], direction=results['flip_direction'][1])
        return results
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.size is not None:
                ...
                for i in range(len(results[key])):
                    results[key][i] = mmcv.impad_to_multiple(
                        results[key][i], self.size_divisor, pad_val=self.pad_val)
            pad_shape = results[key][0].shape
