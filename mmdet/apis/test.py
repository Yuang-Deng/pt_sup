...
def single_gpu_test(model,
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            if flag:
                gen_voc_label(data, result, tags, box, 0.3)
...
def multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    for i, data in enumerate(data_loader):
        ...
        results.extend(result)
        if flag:
            gen_voc_label(data, result, tags, 0.0)
...
def gen_voc_label(data, result, tags, box, pseudo_th=0.9):
    for idx, (d, tags_img, box_img, r) in enumerate(zip(data['img_metas'], tags, box, result)):
        ...
        f = open(os.path.join('/home/qiucm/workspace/dataset/VOCdevkit', os.path.join(d.data[0][0]['filename'].split('/')[6],
